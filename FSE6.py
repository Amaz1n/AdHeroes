...
def instructions():
    ...
    backRect=Rect(50,700,170,70)
    largeFont=font.SysFont("Courier New",70)
    backtext=largeFont.render("Back",True,(255,255,255))
    draw.rect(screen,(160,100,200),backRect)
    screen.blit(backtext,(50,695))
    while running:
        for evnt in event.get():
            if evnt.type == QUIT:
                running = False
        ...
        mb=mouse.get_pressed()
        mx,my=mouse.get_pos()
        if mb[0]==1 and backRect.collidepoint(mx,my) and click:
            return "menu"
        if backRect.collidepoint(mx,my):
            draw.rect(screen,(0,0,0),backRect,2)
        else:
            draw.rect(screen,(200,155,255),backRect,2)
        if key.get_pressed()[27]: running = False
        display.flip()
    return "menu"
