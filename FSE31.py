...
def drawScene(screen,picList1,picList2,health1,health2,bull1,bull2):
    ...
    screen.blit(p2text,(50,10))
    screen.blit(p1text,(850,10))
...
def moveGuy2(pr,cha):
    ...
    maxrapid=7
    if keys[K_c] :#melee frames
        if keyboard2[-1]=="left":#left facing
            newMove=6
        elif keyboard2[-1]=="right":#right facing
            newMove=7
        pr[5]=False
    elif keyboard2[-1]=="left":#if c not pressed and facing left
        newMove=1
    elif keyboard2[-1]=="right":#if c not pressed and facing right
        newMove=2
