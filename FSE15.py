...
def moveGuy2(pr):
    ...
    if keys[K_d] and pr[X]>=0 and keys[K_w] and pr[GODOWN] and pr[DOUBLE]:
        ...
        if pr[Y]<700:
            pr[DOUBLE]=False
    if keys[K_a] and pr[GODOWN]==False or pr[DOUBLE]==False:
        newMove=4
    elif keys[K_a] and pr[X]>=0 and keys[K_w] and pr[GODOWN] and pr[DOUBLE]:
        newMove=4
        pr[VY]=-4
        if pr[Y]<700:
            pr[DOUBLE]=False
    elif keys[K_w] and pr[GODOWN] and pr[DOUBLE]:
        print("jump")
        newMove=3
        pr[VY]=-4
        if pr[Y]<700:
...
pics1=[robotpics,mcman,boomber]
pics2=[robotpics,mcman,boomber]
def game():
    running = True
    while running:
        for evnt in event.get():
